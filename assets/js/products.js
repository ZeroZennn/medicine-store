(async () => {
    ...
    async function addToCart(productId) {
        const checkUserId = carts.cart.findIndex(x => x.user_id == user.id);
        const checkProductId = carts.cart[checkUserId].product.findIndex(x => x.id == productId);
        if (checkProductId >= 0) {
            carts.cart[checkUserId].product[checkProductId].qty++;
        } else {
            carts.cart[checkUserId].product.push({id: parseInt(body.id), qty: 1});
        }
        setLS("carts", carts)
